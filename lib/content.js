...
export const DEFAULT_CONTENT = {
  ...
  // Optional "verify you're human" step. Toggle on/off per form, and choose
  // WHERE it sits (after which step) — to A/B test if it's a roadblock.
  verify: { enabled: true, after: "intro" },
...
// Friendly labels for each step (used by the form progress + Insights).
...
// The fixed steps, minus the optional "verify". Verify is inserted by getFlow.
export const BASE_FLOW = ["intro", "q1", "q2", "q3", "contact", "review", "privacy", "confirm"];

// Steps the "verify" gate is allowed to sit AFTER (for the CMS dropdown).
export const VERIFY_AFTER_OPTIONS = [
  ["intro", "After the intro (Step 2)"],
  ["q1", "After Question 1"],
  ["q2", "After Question 2"],
  ["q3", "After Question 3"],
  ["contact", "After contact info"],
  ["review", "After review (right before submit)"],
];

// Build the ordered list of active steps for a form, inserting the optional
// verify step at its configured position. The public form and the Insights
// funnel both use this, so they always agree.
export function getFlow(content) {
  const flow = [...BASE_FLOW];
  const v = content && content.verify;
  if (!v || v.enabled !== false) {
    const after = (v && v.after) || "intro";
    const i = flow.indexOf(after);
    flow.splice((i >= 0 ? i : 0) + 1, 0, "verify");
  }
  return flow;
}
